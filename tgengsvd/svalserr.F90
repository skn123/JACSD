PROGRAM SVALSERR
  ...
  IF (INFO .NE. 0) THEN
     ...
     STOP 'ERRSTATS(SY)'
  END IF
  WRITE (OUTPUT_UNIT,'(A,6(A,ES25.17E3))') '"SY"', ',',MINAE,',',MAXAE,',',AVGAE,',',MINRE,',',MAXRE,',',AVGRE
  FLUSH(OUTPUT_UNIT)
  ...
  IF (INFO .NE. 0) THEN
     ...
     STOP 'ERRSTATS(SY)'
  END IF
  WRITE (OUTPUT_UNIT,'(A,6(A,ES25.17E3))') '"SW"', ',',MINAE,',',MAXAE,',',AVGAE,',',MINRE,',',MAXRE,',',AVGRE
  FLUSH(OUTPUT_UNIT)
  ...
  IF (INFO .NE. 0) THEN
     ...
     STOP 'ERRSTATS(SY)'
  END IF
  WRITE (OUTPUT_UNIT,'(A,6(A,ES25.17E3))') '"SS"', ',',MINAE,',',MAXAE,',',AVGAE,',',MINRE,',',MAXRE,',',AVGRE
  FLUSH(OUTPUT_UNIT)
...
CONTAINS
  ...
  SUBROUTINE READCL(N, FTRUE, FCOMP, INFO)
    ...
    INFO = 0
    IF (COMMAND_ARGUMENT_COUNT() .NE. 3) STOP 'svalerr.exe N FTRUE FCOMP'
