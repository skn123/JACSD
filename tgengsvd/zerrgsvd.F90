PROGRAM ZERRGSVD
  ...
  COMPLEX(KIND=c_double), ALLOCATABLE :: YW(:,:)
...
CONTAINS
  ...
  SUBROUTINE READCL(FN, M, N, INFO)
    ...
    INFO = 0
    IF (COMMAND_ARGUMENT_COUNT() .NE. 3) STOP 'zerrgsvd.exe FN M N'
  ...
  SUBROUTINE BREAD2(FD, YW, M, N, SZ, INFO)
    ...
    COMPLEX(KIND=c_double), INTENT(OUT) :: YW(M,N)
    ...
    !$OMP DO
    DO J = 1, N
       I = INT(BREADZ(INT(FD,c_int), YW(1,J), INT(SZ,c_size_t), INT(((J-1) * SZ),c_size_t)))
