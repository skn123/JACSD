PROGRAM DERRSVD
  ...
  IF (INFO .NE. 0) THEN
     ...
     STOP ' BIO_OPEN(FNG)'
  END IF
  CALL BIO_READ_D2(U, M, N, A, INFO)
  IF (INFO .NE. 0) THEN
     WRITE (ERROR_UNIT,'(I11)',ADVANCE='NO') INFO
     FLUSH(ERROR_UNIT)
     STOP ' BIO_READ_D2(FNG)'
  END IF
...
CONTAINS
  ...
  SUBROUTINE READCL(M, N, FNG, FNU, FNS, FNV, INFO)
    ...
    INFO = 0
    IF (COMMAND_ARGUMENT_COUNT() .NE. 6) STOP 'derrsvd.exe M N FNG FNU FNS FNV'
