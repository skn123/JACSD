PROGRAM ZGENSVD
  ...
  CALL ZGENDAT(M, N, ISEED, DS, XG, ZG, XWORK, INFO)
  IF (INFO .NE. 0) THEN
     WRITE (ERROR_UNIT,'(I2)',ADVANCE='NO') INFO
     STOP ' ZGENDAT'
  END IF
  ...
  IF (INFO .NE. 0) THEN
     ...
     STOP ' BIO_OPEN(G)'
  END IF
  CALL BIO_WRITE_Z2(U, M, N, ZG, INFO)
  IF (INFO .NE. 0) THEN
     WRITE (ERROR_UNIT,'(I11)',ADVANCE='NO') INFO
     STOP ' BIO_WRITE_Z2(G)'
  END IF
...
CONTAINS
  ...
  SUBROUTINE READCL(ISIGMA, FSIGMA, SEEDI, M, N, FIL, EPS, SCAL, INFO)
    ...
    IF (COMMAND_ARGUMENT_COUNT() .LT. NRQP) THEN
       ...
       WRITE (OUTPUT_UNIT,*) 'FILE.G: double complex(M,N); G = U \Sigma V^H'
       INFO = 1
       RETURN
    END IF
