PROGRAM SSVERR
  ...
  IF (INFO .NE. 0) THEN
     ...
     STOP ' BIO_OPEN(FTRUE)'
  END IF
  CALL BIO_READ_S1(U, N, S, INFO)
  IF (INFO .NE. 0) THEN
     WRITE (ERROR_UNIT,'(I11)',ADVANCE='NO') INFO
     FLUSH(ERROR_UNIT)
     STOP ' BIO_READ_S1(FTRUE)'
  END IF
  CALL BIO_CLOSE(U, INFO)
  IF (INFO .NE. 0) THEN
     WRITE (ERROR_UNIT,'(I11)',ADVANCE='NO') INFO
     FLUSH(ERROR_UNIT)
     STOP ' BIO_CLOSE(FTRUE)'
  END IF
  DO INFO = 1, N
     T(INFO) = S(INFO)
  END DO
  ...
  IF (INFO .NE. 0) THEN
     WRITE (ERROR_UNIT,'(I11)',ADVANCE='NO') INFO
     FLUSH(ERROR_UNIT)
     STOP ' BIO_READ_Q1(FCOMP)'
  END IF
  CALL BIO_CLOSE(U, INFO)
  IF (INFO .NE. 0) THEN
     WRITE (ERROR_UNIT,'(I11)',ADVANCE='NO') INFO
     FLUSH(ERROR_UNIT)
     STOP ' BIO_CLOSE(FCOMP)'
  END IF
  CALL QCSORT(N, C)
  ...
  IF (INFO .NE. 0) THEN
     ...
     STOP ' ERRSTATS'
  END IF
  WRITE (OUTPUT_UNIT,'(5(ES16.9E2,A),ES16.9E2)') MINAE,',',MAXAE,',',AVGAE,',',MINRE,',',MAXRE,',',AVGRE
  FLUSH(OUTPUT_UNIT)
...
CONTAINS
  ...
  SUBROUTINE QCSORT(N, A)
    ...
    EXTERNAL :: PVN_QSORT
    INTEGER(KIND=c_int), EXTERNAL :: QCCMP

    CALL PVN_QSORT(A, INT(N,c_size_t), INT(WP,c_size_t), QCCMP)
  ...
  SUBROUTINE READCL(N, FTRUE, FCOMP, INFO)
    ...
    INFO = 0
    IF (COMMAND_ARGUMENT_COUNT() .NE. 3) STOP 'ssverr.exe N FTRUE FCOMP'
