PROGRAM SGENSVD
  ...
  CALL SGENDAT(M, N, ISEED, SS, QG, SG, QWORK, INFO)
  IF (INFO .NE. 0) THEN
     WRITE (ERROR_UNIT,'(I2)',ADVANCE='NO') INFO
     STOP ' SGENDAT'
  END IF
  ...
  IF (INFO .NE. 0) THEN
     ...
     STOP ' BIO_OPEN(G)'
  END IF
  CALL BIO_WRITE_S2(U, M, N, SG, INFO)
  IF (INFO .NE. 0) THEN
     WRITE (ERROR_UNIT,'(I11)',ADVANCE='NO') INFO
     STOP ' BIO_WRITE_S2(G)'
  END IF
...
CONTAINS
  ...
  SUBROUTINE READCL(ISIGMA, FSIGMA, SEEDI, M, N, FIL, EPS, SCAL, INFO)
    ...
    IF (COMMAND_ARGUMENT_COUNT() .LT. NRQP) THEN
       WRITE (OUTPUT_UNIT,*) 'sgensvd.exe SIGMA SEEDIX M N FILE [ EPS SCALE ]'
       ...
       WRITE (OUTPUT_UNIT,*) 'FILE.G: real(M,N); G = U \Sigma V^T'
       INFO = 1
       RETURN
    END IF
