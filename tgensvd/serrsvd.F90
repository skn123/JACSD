PROGRAM SERRSVD
  ...
  REAL, ALLOCATABLE :: A(:,:)
  REAL(KIND=WP), ALLOCATABLE :: xG(:,:), xU(:,:), xV(:,:), xS(:)
  ...
  IF (INFO .NE. 0) THEN
     ...
     STOP ' BIO_OPEN(FNG)'
  END IF
  CALL BIO_READ_S2(U, M, N, A, INFO)
  IF (INFO .NE. 0) THEN
     WRITE (ERROR_UNIT,'(I11)',ADVANCE='NO') INFO
     FLUSH(ERROR_UNIT)
     STOP ' BIO_READ_S2(FNG)'
  END IF
  ...
  IF (INFO .NE. 0) THEN
     ...
     STOP ' BIO_OPEN(FNV)'
  END IF
  CALL BIO_READ_S2(U, N, N, A, INFO)
  IF (INFO .NE. 0) THEN
     WRITE (ERROR_UNIT,'(I11)',ADVANCE='NO') INFO
     FLUSH(ERROR_UNIT)
     STOP ' BIO_READ_S2(FNV)'
  END IF
  CALL BIO_CLOSE(U, INFO)
  IF (INFO .NE. 0) THEN
     WRITE (ERROR_UNIT,'(I11)',ADVANCE='NO') INFO
...
CONTAINS
  ...
  REAL(KIND=WP) FUNCTION PQGEMM(M, N, K, A, LDA, B, LDB, C, LDC)
    IMPLICIT NONE
  ...
  SUBROUTINE READCL(M, N, FNG, FNU, FNS, FNV, INFO)
    ...
    INFO = 0
    IF (COMMAND_ARGUMENT_COUNT() .NE. 6) STOP 'serrsvd.exe M N FNG FNU FNS FNV'
