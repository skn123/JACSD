PROGRAM ZERRSVD
  ...
  IF (INFO .NE. 0) THEN
     ...
     STOP ' BIO_OPEN(FNG)'
  END IF
  CALL BIO_READ_Z2(U, M, N, A, INFO)
  IF (INFO .NE. 0) THEN
     WRITE (ERROR_UNIT,'(I11)',ADVANCE='NO') INFO
     FLUSH(ERROR_UNIT)
     STOP ' BIO_READ_Z2(FNG)'
  END IF
  ...
  IF (INFO .NE. 0) THEN
     ...
     STOP ' BIO_OPEN(FNV)'
  END IF
  CALL BIO_READ_Z2(U, N, N, A, INFO)
  IF (INFO .NE. 0) THEN
     WRITE (ERROR_UNIT,'(I11)',ADVANCE='NO') INFO
     FLUSH(ERROR_UNIT)
     STOP ' BIO_READ_Z2(FNV)'
  END IF
  CALL BIO_CLOSE(U, INFO)
  IF (INFO .NE. 0) THEN
     WRITE (ERROR_UNIT,'(I11)',ADVANCE='NO') INFO
...
CONTAINS
  ...
  SUBROUTINE READCL(M, N, FNG, FNU, FNS, FNV, INFO)
    ...
    INFO = 0
    IF (COMMAND_ARGUMENT_COUNT() .NE. 6) STOP 'zerrsvd.exe M N FNG FNU FNS FNV'
