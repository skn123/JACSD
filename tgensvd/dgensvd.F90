PROGRAM DGENSVD
  ...
  DOUBLE PRECISION :: EPS, SCAL ! optional
  CHARACTER(LEN=FNL) :: FSIGMA, FIL

  INTEGER :: ISEED(4), U

  DOUBLE PRECISION, ALLOCATABLE :: DS(:)
  REAL(KIND=WP), ALLOCATABLE :: QWORK(:)

  DOUBLE PRECISION, ALLOCATABLE :: DG(:,:)
  REAL(KIND=WP), ALLOCATABLE :: QG(:,:)
  ...
  ALLOCATE(DS(N))
  IF (ISIGMA .NE. 0) THEN
     CALL DGENSV(N, ISEED, ISIGMA, EPS, SCAL, DS, INFO)
  ELSE ! read the file
     CALL DTXTSV(FSIGMA, N, DS, INFO)
  END IF
  IF (INFO .NE. 0) THEN
  ...
  CALL DGENDAT(M, N, ISEED, DS, QG, DG, QWORK, INFO)
  IF (INFO .NE. 0) THEN
     WRITE (ERROR_UNIT,'(I2)',ADVANCE='NO') INFO
     STOP ' DGENDAT'
  END IF
  ...
  IF (INFO .NE. 0) THEN
     ...
     STOP ' BIO_OPEN(G)'
  END IF
  CALL BIO_WRITE_D2(U, M, N, DG, INFO)
  IF (INFO .NE. 0) THEN
     WRITE (ERROR_UNIT,'(I11)',ADVANCE='NO') INFO
     STOP ' BIO_WRITE_D2(G)'
  END IF
...
CONTAINS
  ...
  SUBROUTINE READCL(ISIGMA, FSIGMA, SEEDI, M, N, FIL, EPS, SCAL, INFO)
    ...
    IF (COMMAND_ARGUMENT_COUNT() .LT. NRQP) THEN
       WRITE (OUTPUT_UNIT,*) 'dgensvd.exe SIGMA SEEDIX M N FILE [ EPS SCALE ]'
       ...
       WRITE (OUTPUT_UNIT,*) 'FILE.S: double precision(N); \Sigma'
       WRITE (OUTPUT_UNIT,*) 'FILE.G: double precision(M,N); G = U \Sigma V^T'
       INFO = 1
       RETURN
    END IF
