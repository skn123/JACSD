PROGRAM ZERRHSVD
  ...
  COMPLEX(KIND=c_double), ALLOCATABLE :: Y(:,:)
  REAL(KIND=c_double), ALLOCATABLE :: S(:)
  COMPLEX(KIND=WP), ALLOCATABLE :: xY(:,:), xU(:,:), xZ(:,:)
  ...
  DO J = 1, N
     DO I = 1, M
        xY(I,J) = CMPLX(REAL(Y(I,J)), AIMAG(Y(I,J)), WP)
        ANF = REAL(xY(I,J), WP)
  ...
  DO J = 1, N
     DO I = 1, M
        xU(I,J) = CMPLX(REAL(Y(I,J)), AIMAG(Y(I,J)), WP) * REAL(S(J), WP)
     END DO
  END DO
  !$OMP END PARALLEL DO
  ...
  DO J = 1, N
     DO I = 1, N
        xZ(I,J) = CMPLX(REAL(Y(I,J)), AIMAG(Y(I,J)), WP)
     END DO
  END DO
  !$OMP END PARALLEL DO
...
CONTAINS
  ...
  SUBROUTINE READCL(FN, M, N, INFO)
    ...
    INFO = 0
    IF (COMMAND_ARGUMENT_COUNT() .NE. 3) STOP 'zerrhsvd.exe FN M N'
  ...
  SUBROUTINE BREAD2(FD, Y, M, N, SZ, INFO)
    ...
    COMPLEX(KIND=c_double), INTENT(OUT) :: Y(M,N)
    INTEGER, INTENT(OUT) :: SZ, INFO

    INTEGER :: I, J

    SZ = INT(M * C_SIZEOF(Z_ZERO))
    INFO = 0
    ...
    !$OMP DO
    DO J = 1, N
       I = INT(BREADZ(INT(FD,c_int), Y(1,J), INT(SZ,c_size_t), INT(((J-1) * SZ),c_size_t)))
